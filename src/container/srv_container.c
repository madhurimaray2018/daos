...
ds_cont_op_handler(crt_rpc_t *rpc)
{
	...
	uuid_copy(pool_uuid, pool_hdl->sph_pool->sp_uuid);

	D_DEBUG(DF_DSMS, DF_CONT": processing rpc %p: hdl="DF_UUID" opc=%u\n",
		DP_CONT(pool_uuid, in->ci_uuid), rpc,
...
ds_cont_set_prop_handler(crt_rpc_t *rpc)
{
	int				 rc;
	struct cont_svc			*svc;
	struct cont_prop_set_in		*in = crt_reply_get(rpc);
	...
	/*
	 * Server RPCs don't have pool or container handles, so we need to fetch
	 * the pool and container info.
	 */
	...
	/* If we have a container svc we must be on a pool svc node */
	...
	/* Coming from the server, set-prop is an administrative operation */
	uuid_generate(hdl_uuid);
	rc = ds_cont_local_open(pool_uuid, hdl_uuid, in->cpsi_op.ci_uuid, 0,
				ds_sec_get_admin_cont_capabilities(), &hdl);
	if (rc != 0)
		D_GOTO(out_cont, rc);

	rc = set_prop(&tx, pool, cont, hdl->sch_sec_capas, hdl_uuid,
		      in->cpsi_prop);
	if (rc != 0)
		D_GOTO(out_hdl, rc);
	...
	rc = rdb_tx_commit(&tx);
	if (rc != 0)
		D_ERROR("Unable to commit RDB transaction\n");

out_hdl:
	ds_cont_local_close(hdl_uuid);
out_cont:
	cont_put(cont);
out_lock:
	ABT_rwlock_unlock(svc->cs_lock);
