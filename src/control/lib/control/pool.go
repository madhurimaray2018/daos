...
func genCreateRequest(in *PoolCreateReq) (out *mgmtpb.PoolCreateReq, err error) {
	...
	if out.Numsvcreps == 0 {
		out.Numsvcreps = defaultSvcReps
	}

	// ensure we have a system name in the request
	if out.Sys == "" {
		out.Sys = build.DefaultSystemName
	}

	// ensure pool ownership is set up correctly
	out.User, out.Usergroup, err = formatNameGroup(in.User, in.UserGroup)
	if err != nil {
		return nil, err
	}

	// this doesn't convert automatically
	if !in.ACL.Empty() {
		out.Acl = in.ACL.Entries
	}
...
// Default values for missing request paramters (e.g. owner/group) are generated when
// appropriate.
